        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                envData.roughness = roughness;
                    ...
                    envData.reflUVW = BoxProjectedCubemapDirection(R, i.worldPos, unity_SpecCube0_ProbePosition, unity_SpecCube0_BoxMin, unity_SpecCube0_BoxMax);
                #else
                    envData.reflUVW = R;
                #endif

                float3 env0 = Unity_GlossyEnvironment(UNITY_PASS_TEXCUBE(unity_SpecCube0), unity_SpecCube0_HDR, envData);
                float3 envColor = env0;
                    ...
                    {
                        #if defined(UNITY_SPECCUBE_BOX_PROJECTION)
                            envData.reflUVW = BoxProjectedCubemapDirection(R, i.worldPos, unity_SpecCube1_ProbePosition, unity_SpecCube1_BoxMin, unity_SpecCube1_BoxMax);
                        #else
                            envData.reflUVW = R;
                        #endif
                        float3 env1 = Unity_GlossyEnvironment(UNITY_PASS_TEXCUBE_SAMPLER(unity_SpecCube1, unity_SpecCube0), unity_SpecCube0_HDR, envData);
                        envColor = lerp(env1, env0, blendLerp);
                    }
                #endif
